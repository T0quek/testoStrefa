  ...
  var SmartWizard = /*#__PURE__*/function () {
    ...
    _createClass(SmartWizard, [{
      ...
      value: function _setEvents() {
        ...
        $(this.steps).on("click", function (e) {
          ...
          this.main.find('.sw-btn-next').on("click", function (e) {
              e.preventDefault();
              // Zmienna _this odnosi się do instancji smartWizard

              var currentStep = _this._getStepIndex();
              console.log(currentStep);
              var coursesSelected = $('.multi-select-placeholder-courses').val();
              var setsSelected = $('.multi-select-placeholder-sets').val();

              if (currentStep === 0) {
                  if (!coursesSelected || coursesSelected.length === 0) {
                      toastr.error('Musisz wybrać przynajmniej jeden kurs!');
                      return;
                  }
              ...
              if (currentStep === 1) {
                  if (!setsSelected || setsSelected.length === 0) {
                      toastr.error('Musisz wybrać przynajmniej jeden set!');
                      return;
                  }
                  _this._showNext();
              }
      ...
      value: function _createToolbar(position) {
        ...
        var btnNext = this.options.toolbarSettings.showNextButton !== false ? $('<button></button>').text(this.options.lang.next).addClass('btn btn-outline-primary   sw-btn-next').attr('type', 'button') : null;
        var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false ? $('<button></button>').text(this.options.lang.previous).addClass('btn btn-outline-primary sw-btn-prev').attr('type', 'button') : null;
